...
public:
    ...
    /// @brief Constructor
    explicit notifySubscriptionEvent(para::com::SkeletonInterface* interface) : mInterface(interface)
    {
    }
    /// @brief Destructor
...
public:
    ...
    struct registerSubscriptionAppOutput
    {
        bool returnValue;
    };
    /// @brief Constructor
    ...
    {
        mInterface->SetMethodCallHandler(kregisterSubscriptionAppCallSign, [this](const std::vector<std::uint8_t>& data, const para::com::MethodToken token) {
            HandleregisterSubscriptionApp(data, token);
        });
...
public:
    ...
    /// @uptrace{SWS_CM_00191}
    virtual ara::core::Future<registerSubscriptionAppOutput> registerSubscriptionApp(const eevp::type::String& subscriptionAppName) = 0;
    
private:
    void HandleregisterSubscriptionApp(const std::vector<std::uint8_t>& data, const para::com::MethodToken token)
    {
        std::uint8_t retResult{1};
        std::vector<std::uint8_t> retData{};
        para::serializer::Deserializer deserializer{data};
        eevp::type::String _subscriptionAppName_;
        deserializer.read(_subscriptionAppName_);
        auto future = registerSubscriptionApp(_subscriptionAppName_);
        auto result = future.GetResult();
        if (result.HasValue())
        {
            registerSubscriptionAppOutput output = result.Value();
            para::serializer::Serializer serializer{};
        ...
        mInterface->ReturnMethod(kregisterSubscriptionAppCallSign, retResult, retData, token);
    }
    const std::string kregisterSubscriptionAppCallSign{"registerSubscriptionApp"};
};
} /// namespace skeleton
} /// namespace service
} /// namespace subscription
